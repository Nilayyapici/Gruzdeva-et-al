...
clear all
folderPath = 'C:\Users\Anna\Dropbox\PhD\Cornell\Nilay_Antonio\Photometry\AgRP\M_maze\3sources_4tr_memory_task_all_mice';
...
%% Activity analysis
%% Pearson corr with distance to different sources of food within distance
...
options = struct();
options.dist_lim = 0;
options.diff_clim = [-0.8,0.8];        % Custom color limits
options.exclude_grooming = false;   % Remove grooming timepoints
options.resolution = 40;
options.sigma =4;

options.speed_threshold = 5;
plot_activity_difference_heatmapsM3(mice_all, 'light_control', blueWhiteRed, options);
