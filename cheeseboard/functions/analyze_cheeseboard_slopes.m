function [slope_results] = analyze_cheeseboard_slopes(run_data, options)
    % analyze_cheeseboard_slopes - Analyzes slopes of dF/F traces for cheeseboard maze
    % Compares pre (before) vs test sessions for towards and away runs
    %
    % Inputs:
    %   run_data - Structure with run data from analyze_cheeseboard_runs
    %   options - Structure with optional parameters:
    %       .sessions - Cell array of session names to analyze (default: {'pre', 'test'})
    %       .directions - Cell array of directions (default: {'towards', 'away'})
    %       .max_distance - Maximum distance for analysis (default: 15)
    %       .slope_alpha - Alpha level for significance (default: 0.05)
    %       .min_data_points - Minimum data points for regression (default: 10)
    %       .slope_range - Distance range around food for testing (default: 8 cm)
    %                     For 'towards': tests from -slope_range to 0
    %                     For 'away': tests from 0 to +slope_range
    %       .smoothing - Window size for smoothing before slope calculation (default: 3)
    %       .plot_results - Whether to create plots (default: true)
    %       .expected_direction - 'auto', 'positive', 'negative', or 'none' (default: 'auto')
    %       .bin_width - Width of distance bins for averaging (default: 0.5 cm)
    ...
    if ~isfield(options, 'sessions'), options.sessions = {'pre', 'test'}; end
    ...
    if ~isfield(options, 'slope_range'), options.slope_range = 8; end
    if ~isfield(options, 'smoothing'), options.smoothing = 3; end
    ...
    if ~isfield(options, 'bin_width'), options.bin_width = 0.5; end
    
    % Create session labels for cheeseboard
    session_labels = {'Before', 'Test'};
    
    fprintf('Analyzing cheeseboard slopes for sessions (bin width: %.1f cm): ', options.bin_width);
    ...
    for m = 1:length(run_data)
        ...
        if ~ismember(mouse_sess, options.sessions)
            continue;
        end
        ...
        for d = 1:length(options.directions)
            direction = options.directions{d};
            key = sprintf('%s_%s', mouse_sess, direction);
        ...
        for r = 1:length(runs)
            ...
            for i = 1:length(dist_bins)-1
                ...
                if any(indices)
                    key = sprintf('%s_%s', mouse_sess, run.type);
    ...
    for s = 1:length(options.sessions)
        ...
        for d = 1:length(options.directions)
            ...
            fprintf('\n===== Slope Analysis: %s, %s (±%.1f cm from food) =====\n', ...
            ...
            if strcmp(direction, 'towards')
                ...
                if strcmp(options.expected_direction, 'auto')
                    expected_direction = 'positive'; % Activity increases approaching food (negative distance becomes positive)
                else
            ...
            if length(x_slope) >= options.min_data_points
                [slope_stats] = performCheeseboardMixedEffectsRegression(x_slope, y_slope, key, mouse_data, bin_centers, options);
                ...
                % Store results
                result_key = sprintf('%s_%s', sess, direction);
    ...
    createCheeseboardSlopeSummary(slope_results, options);
    
    % Create plots if requested
    if options.plot_results
        plotCheeseboardSlopeResults(slope_results, options, session_labels);
    end
end

function [slope_stats] = performCheeseboardMixedEffectsRegression(x_slope, y_slope, data_key, mouse_data, bin_centers, options)
    % Perform regression accounting for between-mouse variability for cheeseboard data
...
function createCheeseboardSlopeSummary(slope_results, options)
    ...
    % Print summary
    fprintf('\n===== CHEESEBOARD SLOPE ANALYSIS SUMMARY =====\n');
...
function plotCheeseboardSlopeResults(slope_results, options, session_labels)
    % Create plots showing slope analysis results for cheeseboard maze
    ...
    % Organize data by session and direction
    sessions = {'pre', 'test'};
    ...
    figure('Name', 'Cheeseboard Slope Analysis Results', 'Position', [100, 100, 400*n_directions, 300*n_sessions]);
    
    % Define colors for sessions
    session_colors = {[0.6 0.6 0.6], [0.3 0.3 0.3]}; % Light gray for before, dark gray for test
    ...
    for s = 1:n_sessions
        ...
        for d = 1:n_directions
            ...
            field_name = sprintf('%s_%s', sess_name, direction);
            ...
            if isfield(slope_results.detailed, field_name)
                ...
                % Plot data points
                plot(x_data, y_data, 'o', 'Color', session_colors{s}, 'MarkerSize', 4, ...
                     'DisplayName', sprintf('%s (R²=%.3f)', stats.session_label, stats.r_squared));
                ...
                if stats.is_significant && stats.correct_direction
                    ...
                    plot(x_fit, y_fit, '-', 'Color', session_colors{s}, 'LineWidth', 2, ...
                ...
                if stats.is_significant
                    ...
                    elseif stats.p_value < 0.01
                        significance_text = '**';
                    elseif stats.p_value < 0.05
                        significance_text = '*';
                    end
                    
                    % Add text annotation
                    text(0.1, 0.9, sprintf('p %s', significance_text), 'Units', 'normalized', ...
                         'FontSize', 12, 'FontWeight', 'bold', 'Color', session_colors{s});
                end
            end
            
            % Formatting
            title(sprintf('%s - %s', session_labels{s}, direction));
            if strcmp(direction, 'towards')
    ...
    sgtitle('Cheeseboard Slope Analysis: dF/F vs Distance to Food', 'FontSize', 14);
end