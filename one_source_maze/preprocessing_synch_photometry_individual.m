...
% cd % photometry data folder

% tracking = readtable('.csv'); % file with time and x,y centroid from Bonsai (PC time) from the top view camera
% Analog_table = readtable('.csv'); % file with photometry analog input from Bonsai (PC time)
% 
% DLC = readtable('.csv'); % file with DLC labels
% 
% food_cam = readtable('.csv'); % file with time of frames from food camera

% Example data:
tracking = readtable('F13tg_fasted_food_cam_top2023-12-12T17_08_21.csv'); % write a name of the file with time and x,y centroid; folder should be in the matlab path
Analog_table = readtable('F13tg_fasted_food_AI2023-12-12T17_08_21.csv');

DLC = readtable('F13tg_fasted_food_top2023-12-12T17_08_21DLC_resnet50_MmazeMay11shuffle1_195000.csv');

food_cam = readtable('F13tg_fasted_food_food_cam_time2023-12-12T17_08_21.csv');
...
x1 = 51; % all the corners starting from left top
y1 = 31;
x2 = 119;
y2 = 32;
x3 = 141;
...
figure;
plot(xy_corrected(:, 2), xy_corrected(:, 3))
hold on;
scatter(xy_corrected(:, 2), xy_corrected(:, 3), 15, xy_corrected(:, 6), 'filled');
axis([0 400 0 450]);
% caxis([0 10]);
colormap(jet);
colorbar
...
interaction_food_csv = readtable('food_interaction.csv');
interaction_food_frames = table2array(interaction_food_csv);

eating_csv = readtable('eating.csv');
...
plot([1 1]*(door2), ylim, 'Color', 'b','lineWidth',2);
...
hold all
subplot(1,2,1)
plot(txy_open_door(:,7), txy_open_door(:,6), 'ko', 'MarkerSize', 1)
% % axis([0 230 -2 15])% for path
axis([0 20 -10 13]) % for speed
[rho, pval] = corr(txy_open_door(:,7), txy_open_door(:,6),'Type','Spearman');
title({'Open door', sprintf('Spearman correlation = %f', rho)}, 'FontSize', 14)

subplot(1,2,2)
plot(txy_closed_door(:,7), txy_closed_door(:,6), 'ko', 'MarkerSize', 1)
% axis([0 230 -2 15]) % for path
axis([0 20 -10 13]) % for speed
[rho, pval] = corr(txy_closed_door(:,7), txy_closed_door(:,6),'Type','Spearman');
title({'Closed door door', sprintf('Spearman correlation = %f', rho)}, 'FontSize', 14)
